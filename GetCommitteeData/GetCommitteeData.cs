    ...
    //get detailed committee data for each candidate, uses poly to rate limit requests because of limitations in free API (1000 requests per hour)
    {
        ...
        public async Task Run([TimerTrigger("0 */2 * * * *")] TimerInfo myTimer, ILogger log)
        {
            ...
            foreach (var candidate in candidateIDs)
            {
                ...
                log.LogInformation("Getting information for candidate: {1}", candidate.Body.ToString());
                bool result = await SharedComponents.PollyPolicy.GetDefault.ExecuteAsync(() => committeeSearcher.Submit());
                if (!result)
                {
